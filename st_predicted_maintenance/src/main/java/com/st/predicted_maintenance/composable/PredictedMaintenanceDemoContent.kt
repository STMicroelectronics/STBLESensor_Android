...
fun PredictedMaintenanceDemoContent(
    ...
    Column(
        modifier = modifier.fillMaxWidth().padding(LocalDimensions.current.paddingNormal).verticalScroll(rememberScrollState()),
        verticalArrangement = Arrangement.spacedBy(LocalDimensions.current.paddingLarge),
        horizontalAlignment = Alignment.Start
    ) {
        speedData?.let { speed ->
            PredictedMaintenanceElementView(title = "RMS Speed Status", value = speed, format = "RMS Speed (mm/s): %.2f")
        }

        accData?.let { acc ->
            PredictedMaintenanceElementView(title = "Acceleration Peak Status", value = acc, format = "Acc Peak (m/s^2): %.2f")
        }

        freqData?.let { freq ->
            PredictedMaintenanceElementView(title = "Frequency Domain Status", value = freq, format = "Max amplitude (m/s^2): %.2f")
        }
    }
}