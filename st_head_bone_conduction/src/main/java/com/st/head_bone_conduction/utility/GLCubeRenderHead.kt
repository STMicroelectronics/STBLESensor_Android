...
internal class GLCubeRenderHead(
    c: Context?,
    color: Int
) : GLSurfaceView.Renderer {

    private val TAG = GLCubeRenderHead::class.java.canonicalName
    ...
    private val mTempModelMatrix = FloatArray(16) // To avoid .clone() and GC pressure
    ...
    private val autoResetThreshold = 25//45

    // Shader handles
    private var mProgramHandle = 0
    private var mMVPMatrixHandle = -1
    private var mModelMatrixHandle = -1
    private var mPositionHandle = -1
    private var mNormalHandle = -1
    private var mMaterialKaHandle = -1
    private var mMaterialKdHandle = -1
    private var mMaterialKsHandle = -1
    private var mMaterialNsHandle = -1
    private var mLightDirectionHandle = -1
    private var mLightColorHandle = -1
    private var mAmbientStrengthHandle = -1
    ...
    // Rotation and Reset (Quaternions)
    private val qCurrent = floatArrayOf(0f, 0f, 0f, 1f)
    private val qReference = floatArrayOf(0f, 0f, 0f, 1f)
    private val mRelativeRotationMatrix = FloatArray(16)

    // Rendering rate (FPS) calculation
    ...
    init {
        ...
        Matrix.setIdentityM(mRelativeRotationMatrix, 0)
    ...
    override fun onSurfaceCreated(glUnused: GL10?, config: EGLConfig?) {
        GLES20.glClearColor(
            Color.red(mBgColor) / 255f,
            Color.green(mBgColor) / 255f,
            Color.blue(mBgColor) / 255f,
            Color.alpha(mBgColor) / 255f
        )
    ...
    override fun onDrawFrame(glUnused: GL10?) {
        ...
        // Pass material and light properties to shader
        ...
        // Compute rotation only if motion is enabled
        if (isMotionEnabled) {
            synchronized(this) {
                computeRelativeRotationMatrix(qCurrent, qReference, mRelativeRotationMatrix)
            }
            // Apply rotation matrix without creating new objects
            System.arraycopy(mModelMatrix, 0, mTempModelMatrix, 0, 16)
            Matrix.multiplyMM(mModelMatrix, 0, mTempModelMatrix, 0, mRelativeRotationMatrix, 0)
        }

        // Apply scale
        val modelBaseScale = 1.0f
        Matrix.scaleM(mModelMatrix, 0, scale_cube * modelBaseScale, scale_cube * modelBaseScale, scale_cube * modelBaseScale)
    ...
    /**
     * Computes a 4x4 rotation matrix that represents the orientation 'qIn' 
     * relative to a calibration point 'qRef'.
     *
     * This uses the formula: qRel = qRef_inverse * qIn
     * It effectively re-centers the coordinate system so that qRef becomes the identity.
     */
    private fun computeRelativeRotationMatrix(qIn: FloatArray, qRef: FloatArray, outMatrix: FloatArray) {
        // 1. Calculate the inverse of the reference quaternion (conjugate for unit quaternions)
        val invX = -qRef[0]; val invY = -qRef[1]; val invZ = -qRef[2]; val invW = qRef[3]

        // 2. Multiplication: qRel = qRef_inv * qIn
        // This treats qRef as the new local frame origin.
        val rx = invW * qIn[0] + invX * qIn[3] + invY * qIn[2] - invZ * qIn[1]
        val ry = invW * qIn[1] - invX * qIn[2] + invY * qIn[3] + invZ * qIn[0]
        val rz = invW * qIn[2] + invX * qIn[1] - invY * qIn[0] + invZ * qIn[3]
        val rw = invW * qIn[3] - invX * qIn[0] - invY * qIn[1] - invZ * qIn[2]

        // 3. Direct conversion to 4x4 column-major Matrix
        val xx = rx * rx; val yy = ry * ry; val zz = rz * rz
        val xy = rx * ry; val xz = rx * rz; val yz = ry * rz
        val wx = rw * rx; val wy = rw * ry; val wz = rw * rz

        outMatrix[0] = 1f - 2f * (yy + zz); outMatrix[1] = 2f * (xy + wz);   outMatrix[2] = 2f * (xz - wy);   outMatrix[3] = 0f
        outMatrix[4] = 2f * (xy - wz);     outMatrix[5] = 1f - 2f * (xx + zz); outMatrix[6] = 2f * (yz + wx);   outMatrix[7] = 0f
        outMatrix[8] = 2f * (xz + wy);     outMatrix[9] = 2f * (yz - wx);   outMatrix[10] = 1f - 2f * (xx + yy); outMatrix[11] = 0f
    ...
    private fun updateRenderingRate() {
        ...
        val currentRate = if (period == 0L) MAX_HW_RENDERING_RATE_HZ else (1000 / period).toInt()
        rendering_start_time = now
        // Simple sliding average for FPS calculation
    ...
    fun getRenderingRate(): Int = minOf((rendering_rate_avg / RENDERING_RATE_WINDOW_SIZE).roundToInt(), MAX_HW_RENDERING_RATE_HZ)

    fun setScaleCube(scale: Float) {
        scale_cube = SCALE_CUBE_MIN + SCALE_CUBE_FACTOR * scale
    }

    fun setBoardId(id: Int) {
        boardIDint = id
        Log.d(TAG, "Board ID set to: $boardIDint")
    }

    /**
     * Updates rotation and manages the initial "frozen" period with continuous reset.
     */
    fun setRotation(qi: Float, qj: Float, qk: Float, qs: Float) {
        Log.d(TAG, "Incoming Quaternions -> qi: $qi, qj: $qj, qk: $qk, qs: $qs")
        Log.d(TAG, "ADV_Name -> Name: $boardIDint")
        synchronized(this) {
            // Axis mapping based on board type
            when {
                boardIDint == 0xD1 || boardIDint == -47 -> {
                    qCurrent[0] = -qk
                    qCurrent[1] = qj
                    qCurrent[2] = -qi
                    qCurrent[3] = -qs
                }
                boardIDint == 0xD2 || boardIDint == -46 -> {
                    qCurrent[0] = -qk
                    qCurrent[1] = qj
                    qCurrent[2] = qi
                    qCurrent[3] = qs
            ...
            // Initial stabilization gate
            if (!isMotionEnabled) {
                sampleCounter++
                
                // Continuous reset to keep the face locked to current data until threshold is met
                System.arraycopy(qCurrent, 0, qReference, 0, 4)
                
                if (sampleCounter >= autoResetThreshold) {
                    Log.d(TAG, "Incoming Quaternions -> STABILIZATION COMPLETE AT SAMPLE $sampleCounter. TRIGGERING RESET.")
                    resetCube() // Trigger final calibration reset and unlock motion
                }
            }
        }
    }
    ...
    /**
     * Sets current orientation as zero reference and enables motion.
     * This is the function used for both auto-reset and the manual "Reset Cube" button.
     */
    fun resetCube() {
        Log.d(TAG, "Incoming Quaternions -> RESET")
        synchronized(this) {
            System.arraycopy(qCurrent, 0, qReference, 0, 4)
    ...
    /**
     * Resets the stabilization gate for a fresh start.
     */
    fun hideModel() {
        synchronized(this) {
    ...
    private fun loadObjFromRawResource(@RawRes resId: Int): ObjModel {
        val reader = BufferedReader(InputStreamReader(mContext!!.resources.openRawResource(resId)))
        ...
        reader.useLines { lines ->
            lines.forEach { line ->
                ...
                when (p[0]) {
                    "v" -> { tempV.add(p[1].toFloat()); tempV.add(p[2].toFloat()); tempV.add(p[3].toFloat()) }
                    "vn" -> { tempN.add(p[1].toFloat()); tempN.add(p[2].toFloat()); tempN.add(p[3].toFloat()) }
                    "f" -> {
    ...
    private fun addV(tok: String, tv: List<Float>, tn: List<Float>, fv: MutableList<Float>, fn: MutableList<Float>) {
        val idx = tok.split('/')
        val vI = idx[0].toInt() - 1
        fv.add(tv[vI * 3]); fv.add(tv[vI * 3 + 1]); fv.add(tv[vI * 3 + 2])
        if (idx.size > 2 && idx[2].isNotEmpty()) {
            val nI = idx[2].toInt() - 1
            fn.add(tn[nI * 3]); fn.add(tn[nI * 3 + 1]); fn.add(tn[nI * 3 + 2])
        } else {
            fn.add(0f); fn.add(0f); fn.add(1f)
        }
    }
    ...
    private fun loadMtlFromRawResource(@RawRes resId: Int) {
        ...
        reader.useLines { lines ->
            lines.forEach { line ->
                ...
                when (p[0]) {
                    "Ka" -> if (p.size >= 4) { materialKa[0] = p[1].toFloat(); materialKa[1] = p[2].toFloat(); materialKa[2] = p[3].toFloat() }
                    "Kd" -> if (p.size >= 4) { materialKd[0] = p[1].toFloat(); materialKd[1] = p[2].toFloat(); materialKd[2] = p[3].toFloat() }
                    "Ks" -> if (p.size >= 4) { materialKs[0] = p[1].toFloat(); materialKs[1] = p[2].toFloat(); materialKs[2] = p[3].toFloat() }
                    "Ns" -> if (p.size >= 2) { materialNs = p[1].toFloat() }
                }
            }
        }
    }
