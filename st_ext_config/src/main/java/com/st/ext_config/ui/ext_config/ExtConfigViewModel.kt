...
@Inject internal constructor(
    ...
    var banksStatus: BanksStatus? = null
                    ...
                    features.filterIsInstance<ExtConfiguration>().firstOrNull()?.let { feature ->
                        ...
                        initialCommands.forEach { command ->
                            ...
                            if (response is ExtendedFeatureResponse) {
                                ...
                                response.response.banksStatus?.let {
                                    banksStatus = it
                                }
                            }
                        }
            ...
            if (features.isNotEmpty()) {
                ...
                if (feature != null) {
                    val featureCommand = when (command) {
                        ...
                        Command.SET_CERTIFICATE -> null // Not supported by new ST Ble Sensor:
...
enum class Command(val commandName: CommandName) {
    ...
    companion object {
        fun fromValue(value: String): Command? {
            return entries.find { it.commandName.value == value }
        }
    }
}
